...
test('Need INVITE_USER to be able to invite user', () => {
  const user = permissions.GRANT_PERMISSION;
  expect(acu(user).check(permissions.INVITE_USER)).toBe(false);
});
...
test('User with INVIUTE_USER permission and DELETE_DOCUMENT should be granted access to INVITE_USER feature', () => {
  const user = permissions.INVITE_USER | permissions.DELETE_DOCUMENT;
  const access = acu(user).check(permissions.INVITE_USER);
  expect(access).toBe(true);
});
...
test('User with all regular permission except INVITE_USER must NOT be granted access to INVITE_USER feature', () => {
  const user = permissions.DELETE_DOCUMENT | permissions.REMOVE_USER;
  const access = acu(user).check(permissions.INVITE_USER);
  expect(access).toBe(false);
});
...
test('user with negative permission number must not have access to anything', () => {
  ...
  access = acu(user).check(permissions.DELETE_DOCUMENT);
  expect(access).toBe(false);
...
test('User without grant permission fails to grant new permissions', () => {
  ...
  user = permissions.DELETE_DOCUMENT;
  access = acu(user).canSet(permissions.INVITE_USER);
  expect(access).toBe(false);
